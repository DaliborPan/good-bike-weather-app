...
const pretifyMessage = (message: string) => message.charAt(0).toUpperCase() + message.slice(1)
...
export const EntityDetail: FC<Props> = ({ className = '', open, onClose, dayData }) => {
  return (
    <Dialog className={clsx('w-[50rem] h-[26rem] max-w-[80vw] max-h-[90vh]', className)} open={open} onClose={onClose}>
        ...
        <div className="flex flex-col grow items-center justify-around">
          ...
          <div className="w-11/12 flex justify-around">
            <MetricVisualisation
              ...
              unit={'°C'}
            />
            <MetricVisualisation Icon={IcoRain} fromValue={dayData?.precipitation ?? '0'} unit="mm" />
          </div>
            ...
            <div className="bg-light-green w-full rounded-lg grow p-3 overflow-auto">
              {dayData?.accidents.map((accident, index) => (
                <div key={index} className="flex gap-2 mb-2">
                  ...
                  <div>
                    {pretifyMessage(accident.attributes.nasledky)}
                    <br />
                    <span className="text-black/70 text-xs">{pretifyMessage(accident.attributes.pricina)}</span>
                  </div>
                </div>
              ))}
