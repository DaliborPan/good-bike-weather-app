...
export const Toolbar: FC<IProps> = ({ className, onChange }) => {
  ...
  return (
    <div className={`bg-light-green rounded-lg h-16 flex justify-center items-center gap-4 px-5 ${className ? className : ''}`}>
      <DatePicker
        className={'basis-40'}
        label="Date from"
        value={dateFrom}
        onChange={(val) => { setDateFrom(val as Moment); }}
      ...
      <DatePicker
        ...
        className={'basis-40'}
        value={dateTo}
        onChange={(val) => { setDateTo(val as Moment); }}
        renderInput={(params) => <TextField {...params} variant={'standard'}/>}
      />
      <Box className={'basis-1/8 grow px-5 h-12'}>
          <InputLabel variant='standard' shrink>Temperature</InputLabel>
          <Slider
        ...
        <Box className={'basis-1/8 grow px-5 h-12'}>
          <InputLabel variant='standard' shrink>Precipitation</InputLabel>
          <Slider
        ...
        <Box className={'basis-1/8 grow px-5 h-12'}>
          <InputLabel variant='standard' shrink>Risk index</InputLabel>
          <Slider
        ...
        <FormControl variant='standard' className={'basis-40 h-12'}>
          ...
          <Select
            ...
            value={transport}
            multiple
            onChange={(e) => setTransport(typeof e.target.value === 'string' ? e.target.value.split(',') : e.target.value)}
          >
