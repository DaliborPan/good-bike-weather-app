  ...
  return (
    <div className="flex flex-col absolute top-0 bottom-0 left-0 right-0 app-bg items-center gap-5 pb-8">
        ...
        <Tab.List as="div" className={'h-12 w-full text-lg flex'}>
            <Tab className={({ selected }) => `basis-1/2 transition-all ${ selected ? '' : 'bg-whiskey text-white' }`}>Table</Tab>
            <Tab className={({ selected }) => `basis-1/2 transition-all ${ selected ? '' : 'bg-whiskey text-white' }`}>Chart</Tab>
        </Tab.List>
        <Tab.Panels>
